...
class ONE(nn.Module):
    def __init__(self, input_shape,  output_shape):
        ...
        self.decoder0 = Decoder(input_shape, output_shape)
        self.decoder1 = Decoder(input_shape, [1, 1])
        self.decoder2 = Decoder(input_shape, [1, 1])
        self.decoder3 = Decoder(input_shape, [1, 1])

    def forward(self, x):
        encoder = self.encoder(x)
        close_volume_delta = self.decoder0(encoder)
        three = self.decoder1(encoder)
        seven = self.decoder2(encoder)
        thirty_one = self.decoder3(encoder)
        return close_volume_delta, three, seven, thirty_one
