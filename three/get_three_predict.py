...
def get_exchange_growth_death(exchange):
    ...
    #截取zero股票里大于median值的股票列表
    zero_data = zero_data[zero_data['three'] > zero_data['three'].median()].reset_index(drop=True)
    zero_list = zero_data['symbol'].to_list()
    # predict结果
    predict_list = []
    ...
    for i in tqdm(range(len(groups))):
        ...
        one, three, seven= model(symbol_data)
        one = one.cpu().detach().numpy()[0][0][0]
        three = three.cpu().detach().numpy()[0][0][0]
        seven = seven.cpu().detach().numpy()[0][0][0]
        predict_result = {'symbol': symbol, 'endDate': int(endDate), 'one': one, 'three': three, 'seven': seven}
        predict_list.append(predict_result)
    predict_data = pd.DataFrame(predict_list)
    predict_data['growth_death'] = predict_data['one'] + predict_data['three'] + predict_data['seven']
