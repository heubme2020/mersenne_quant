...
class Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        input_data, dcf_three, dcf_seven, dcf_thirty_one = get_data_input(data)
        input_data = input_data.values
        input_data = torch.tensor(input_data).float()
        dcf_three = torch.tensor(dcf_three).unsqueeze(0).unsqueeze(0).float()
        dcf_seven = torch.tensor(dcf_seven).unsqueeze(0).unsqueeze(0).float()
        dcf_thirty_one = torch.tensor(dcf_thirty_one).unsqueeze(0).unsqueeze(0).float()
        return input_data, dcf_three, dcf_seven, dcf_thirty_one


def get_data_input(data):
    input_data = data.iloc[:, :-3]
    dcf_three = data.iloc[0, -3]
    dcf_seven = data.iloc[0, -2]
    dcf_thirty_one = data.iloc[0, -1]
    return input_data, dcf_three, dcf_seven, dcf_thirty_one
...
def train_seven_model():
    ...
    model = SEVEN([127, 31], [1, 1]).to(device)
    batch_size = 128
    train_folder = os.path.join(current_dir, 'train')
    h5_files = [os.path.join(train_folder, f) for f in os.listdir(train_folder) if f.endswith('.h5')]
    random.shuffle(h5_files)
    ...
    for epoch in range(num_epochs):
        ...
        mean_train_loss = 0.0
        step_num = 0
        for data_input, three, seven, thirty_one in train_dataloader:
            ...
            thirty_one = thirty_one.to(device, non_blocking=True)
            optimizer.zero_grad()
            three_predict, seven_predict, thirty_one_predict = model(data_input)
            three_loss = criterion_mse(three, three_predict)
            seven_loss = criterion_mse(seven, seven_predict)
            thirty_one_loss = criterion_mse(thirty_one, thirty_one_predict)
            loss = three_loss + seven_loss + thirty_one_loss
            # loss = criterion(predict0, fore0) + criterion(predict1, fore1) + 3*criterion(compare, gain)
            # print(criterion(compare, gain).item())
            loss.backward()  # 计算梯度
            # optimizer.step()
        ...
        mean_val_loss = 0
        with ((torch.no_grad())):
            for data_input, three, seven, thirty_one in val_dataloader:
                data_input = data_input.to(device, non_blocking=True)
                three = three.to(device, non_blocking=True)
                seven = seven.to(device, non_blocking=True)
                thirty_one = thirty_one.to(device, non_blocking=True)
